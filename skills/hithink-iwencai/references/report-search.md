...
$headers = @{
    ...
    "login-token" = "$env:117862897_login_token"
...
4. **API调用执行**: 生成并执行API调用代码，使用login-token认证和X-Claw-* Header
5. **数据质量评估**: 检查返回的研究报告是否专业、完整，能否回答用户问题
6. **专业数据处理**: 对搜索结果进行专业排序、过滤、摘要和关键信息提取
7. **结果整合返回**: 将处理后的专业结果返回给大模型，生成深度回答

### 代码生成要求
- 生成完整的API调用代码，包括login-token认证、X-Claw-* Header、64字符Trace ID
...
2. **认证合规**: 必须使用login-token认证方式，符合公网访问要求
